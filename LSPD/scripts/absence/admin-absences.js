...
async function loadStats() {
    try {
        ...
        document.getElementById('total-absences').textContent = stats.total_absences || 0;
        document.getElementById('pending-absences').textContent = stats.en_attente || 0;
        document.getElementById('approved-absences').textContent = stats.approuvees || 0;
        document.getElementById('rejected-absences').textContent = stats.refusees || 0;
...
function generateCalendar() {
    ...
    for (let i = 1; i < 43; i++) {
        ...
        dayElement.addEventListener('click', () => showDayDetails(date, dayAbsences));
...
function displayRequests() {
    ...
    if (filteredAbsences.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: #666; padding: 40px;">Aucune demande d\'absence trouvée</p>';
        return;
    }
...
function createRequestCard(absence) {
    ...
    card.innerHTML = `
        <div class="request-header">
            <div class="request-agent">${absence.officier} (${absence.grade})</div>
            <div class="request-status ${absence.statut}">${statusText[absence.statut]}</div>
        </div>
    ...
    modalBody.innerHTML = `
        <div class="detail-grid">
            ...
            <div class="detail-item">
                ...
                <div class="detail-value">${typeText[absence.type_absence]}</div>
            </div>
            <div class="detail-item">
            ...
            <div class="detail-item">
                ...
                <div class="detail-value">${formatDate(absence.date_debut)} ${absence.heure_debut || ''}</div>
            </div>
            <div class="detail-item">
                <div class="detail-label">Date de fin</div>
                <div class="detail-value">${formatDate(absence.date_fin)} ${absence.heure_fin || ''}</div>
            </div>
        </div>
        ...
        <div class="detail-item" style="margin-bottom: 15px;">
            ...
            <div class="detail-value">${absence.justificatif ? 'Oui' : 'Non'}</div>
        </div>
...
async function confirmStatusChange() {
    ...
    try {
        ...
        showSuccess(`Demande ${pendingStatusChange.action === 'approuver' ? 'approuvée' : 'refusée'} avec succès`);
...
function applyFilters() {
    ...
    filteredAbsences = allAbsences.filter(absence => {
        ...
        if (dateStart && new Date(absence.date_debut) < new Date(dateStart)) return false;
...
function showDayDetails(date, absences) {
    ...
    modalTitle.textContent = `Absences du ${formatDate(date.toISOString().split('T')[0])}`;

    let content = '<div class="day-absences">';
    absences.forEach(absence => {
        content += `
            <div class="absence-summary" onclick="showAbsenceDetails(${JSON.stringify(absence).replace(/"/g, '&quot;')})">
                <strong>${absence.officier}</strong> (${absence.grade}) - ${absence.type_absence}
...
function setupEventListeners() {
    ...
    ['filter-status', 'filter-type', 'filter-date-start', 'filter-date-end'].forEach(id => {
