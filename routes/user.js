...
// Route pour récupérer tous les membres Discord du serveur LSPD
router.get('/api/discord/members', checkAuth, async (req, res) => {
  try {
    const guild = await bot.guilds.fetch(GUILD_ID);

    // Récupérer tous les membres (force fetch pour avoir les données à jour)
    await guild.members.fetch();

    // Filtrer pour ne récupérer que les membres avec des rôles LSPD
    const conf = await getConfig();
    const { rows } = await pool.query('SELECT * FROM lspd_grades LIMIT 1');
    const gradeRow = rows[0];

    // Liste de tous les rôles LSPD
    const lspdRoleIds = [
      gradeRow.rookie_role_id,
      gradeRow.officier_1_role_id,
      gradeRow.officier_2_role_id,
      gradeRow.officier_3_role_id,
      gradeRow.slo_role_id,
      gradeRow.sergent_1_role_id,
      gradeRow.sergent_2_role_id,
      gradeRow.sergent_chef_role_id,
      gradeRow.lieutenant_role_id,
      gradeRow.lieutenant_chef_role_id,
      gradeRow.capitaine_role_id,
      gradeRow.commandant_role_id,
      gradeRow.chief_role_id
    ].filter(Boolean);

    const members = [];
    guild.members.cache.forEach(member => {
      // Vérifier si le membre a au moins un rôle LSPD
      const hasLspdRole = member.roles.cache.some(role => lspdRoleIds.includes(role.id));
    ...
    // Trier par displayName
    members.sort((a, b) => a.displayName.localeCompare(b.displayName));

    res.json(members);
  } catch (error) {
    console.error('Erreur GET /api/discord/members:', error);
    ...
    try {
      ...
      if (!hasRequiredRole && !isDOJ) {
        ...
        try {
            ...
            if (config.logs_connexion) {
              try {
                ...
                if (logsChannel?.isTextBased()) {
                  ...
                  const desc = roleId
                    ? `${account.username} a tenté(e) de se connecter et est blacklisté (rôle <@&${roleId}>)`
                    : `${account.username} a tenté(e) de se connecter et est blacklisté`;
          ...
          console.error('Erreur check blacklist DB (login-local):', e);
        }
